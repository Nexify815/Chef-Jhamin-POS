...
export default function EditUserModal({ user, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showAlert('error', 'Error', err.message || 'Failed to update user');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(8px)' }} onClick={onClose}>
      <div className="glass-panel w-[90%] max-w-[500px] max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-teal">Edit User</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white text-2xl leading-none">&times;</button>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-teal-light text-sm font-medium mb-1">Full Name</label>
            <input type="text" value={fullname} onChange={e => setFullname(e.target.value)} className="input-field w-full" required />
          </div>

          <div>
            <label className="block text-teal-light text-sm font-medium mb-1">Username</label>
            <input type="text" value={username} onChange={e => setUsername(e.target.value)} className="input-field w-full" required />
          </div>

          <div>
            <label className="block text-teal-light text-sm font-medium mb-1">Role</label>
            <CustomSelect value={role} onChange={setRole} options={['staff', 'owner']} placeholder="Select role" required />
          </div>

          <div>
            <label className="block text-teal-light text-sm font-medium mb-1">Password <span className="text-gray-500">(leave empty to keep current)</span></label>
